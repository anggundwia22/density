...
void loop()
{
    ...
    // Read sensor continuously to update current angle and density in real-time
    if (measurementState == IDLE || measurementState == MEASURING)
    {
        ...
        // Update current angle (calibration will be applied in angleToDensity)
        currentAngle = rawAngle;

        // Calculate density from angle (calibration applied inside angleToDensity)
        currentDensity = angleToDensity(rawAngle);
    }
...
float angleToDensity(float angle)
{
    ...
    float angle1 = 0.0;
    float density1 = 1.000;
    float angle2 = 45.0;
    float density2 = 1.120;

    float density = density1 + (calibratedAngle - angle1) * ((density2 - density1) / (angle2 - angle1));
